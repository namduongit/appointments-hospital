...
const AdminTablePagination = (props: AdminTablePagination) => {
    ...
    return (
        <div className="flex justify-between w-full px-4 py-2 mt-3">
            ...
            <div className="flex gap-2 items-center">
                ...
                {
                    Array.from({ length: (props.array.length / row) }, (_, i) => (
                        <button 
                            key={i}
                ...
                <button className={`border text-center w-7 h-7 text-md rounded font-semibold 
                transition-colors ${page === props.array.length ? 'border-gray-600 text-gray-300 bg-gray-600' : 'border-gray-300 text-gray-600'}`}
                    disabled={page === props.array.length}
                >
